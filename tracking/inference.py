...
class InferenceModule:
    ...
    def initialize(self, gameState):
        ...
        self.legalPositions = [p for p in gameState.getWalls().asList(False) if p[1] > 1] #what does p[1] > 1?????????????????????????
...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for possible_previous_position in self.legalPositions:
            new_position_distribution = self.getPositionDistribution(self.setGhostPosition(gameState, possible_previous_position))
            for new_position, p_newPosition_given_possiblePreviousPosition in new_position_distribution.items():
                probability_distribution_for_positions_after_time_elapse[new_position] += p_newPosition_given_possiblePreviousPosition * self.beliefs[possible_previous_position]
