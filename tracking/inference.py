...
class InferenceModule:
    ...
    def initialize(self, gameState):
        ...
        self.legalPositions = [p for p in gameState.getWalls().asList(False) if
                               p[1] > 1]  # what does p[1] > 1?????????????????????????
...
class ExactInference(InferenceModule):
        ...
        if noisyDistance is None:
            ...
            for p in self.legalPositions:
                ...
                if emissionModel[trueDistance] > 0:
                    # referenced: HMMS slides Page30
                    #   Pr(Xt=p |e_t, e_{t-1}, e_{t-2}, ..., e_1)
                    # = Pr(Xt=p | e_{t-1}, e_{t-2}, ..., e_1) * Pr(noisyDistance | Xt=p)
                    # = Pr(Xt=p | e_{t-1}, e_{t-2}, ..., e_1) * Pr(noisyDistance | trueDistance)
                    # = Pr(Xt=p | e_{t-1}, e_{t-2}, ..., e_1) * Pr(noisyDistance | manhattanDistance(p,packmanPosition))
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        """
        "*** YOUR CODE HERE ***"
        probability_distribution_for_positions_after_time_elapse = util.Counter()
        for cur_position in self.legalPositions:
            prob_distribution_over_next_positions = self.getPositionDistribution(self.setGhostPosition(gameState, cur_position))
            # referenced: HMMs slides Page27
            #   P(Next_Pos = next_pos | e_{t-1}, e_{t_2} ..., e_1)
            # = summation over all possible cur_pos: (Pr(next_pos|cur_pos) * Pr(cur_pos|e_{t-1}, e_{t-2} ... e_1))
            # = summation over all possible cur_pos: (p_nextPosition_given_curPosition * self.beliefs[cur_pos]
            for next_position, p_nextPosition_given_curPosition in prob_distribution_over_next_positions.items():
                probability_distribution_for_positions_after_time_elapse[
                    next_position] += p_nextPosition_given_curPosition * self.beliefs[cur_position]
        probability_distribution_for_positions_after_time_elapse.normalize()
        self.beliefs = probability_distribution_for_positions_after_time_elapse
        "*** END YOUR CODE HERE ***"
